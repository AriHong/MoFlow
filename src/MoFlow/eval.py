...
def get_genescore(adata, nz_threshold=0.1, w_threshold=90):
    ...
    m1_ = np.zeros(adata.n_vars)
    m2_ = np.zeros(adata.n_vars)
    
    for i, g in enumerate(adata.var_names):
        c = adata[:, g].layers['c'].ravel()
        u = adata[:, g].layers['u'].ravel()
        s = adata[:, g].layers['s'].ravel()
        
        vc = adata[:, g].layers['velo_c'].ravel()
        vs = adata[:, g].layers['velo_s'].ravel()
        vu = adata[:, g].layers['velo_u'].ravel()
        
        w_non_zero = ((c >= nz_threshold * np.max(c)) &
                        (u >= nz_threshold * np.max(u)) &
                        (s >= nz_threshold * np.max(s)))
        u_non_zero = u[w_non_zero]
        s_non_zero = s[w_non_zero]
        wu = u >= np.percentile(u_non_zero, w_threshold)
        ws = s >= np.percentile(s_non_zero, w_threshold)
